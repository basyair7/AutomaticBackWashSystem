...
void FSProgram::updateRelay(const String& cfile, std::function<void (StaticJsonDocument<500>&)> updateFunc)
{
...
void FSProgram::changeVarRelay(String varName, String value) {
    updateRelay(configRelay, [&](StaticJsonDocument<500>& doc) {
        doc[varName]["name"] = value;
    });
}

void FSProgram::changeStateRelay(String varName, bool status) {
    updateRelay(configRelay, [&](StaticJsonDocument<500>& doc) {
        doc[varName]["status"] = status;
    });
}

void FSProgram::changePinRelay(String varName, uint8_t _pin) {
    updateRelay(configRelay, [&](StaticJsonDocument<500>& doc) {
        doc[varName]["pin"] = _pin;
    });
}

void FSProgram::parseVarRelay(const String& varName, int* id, int* pin, String* name, bool* status)
{
    StaticJsonDocument<500> doc;
    file = LittleFS.open(configRelay, "r");
    if (!file) {
